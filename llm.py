...
class LLM:
    ...
    def __init__(
        ...
        Args:
            ...
            system_prompt: Optional system prompt to prepend to chats.
    ...
    def batch_generate(
        ...
        message_batches = [
            self._build_messages(prompt, use_history)
        ...
        # Clear KV cache to prevent OOM
        # Note: We intentionally do NOT call torch.cuda.empty_cache() here
        # as it causes severe performance degradation (cache thrashing/fragmentation)
        # when called frequently. KV cache clearing alone is sufficient.
